...
import { mergeHasDigested, mergeInterviews } from './merge';
...
export class Interviewer {
  ...
  private readonly DANGEROUS_ENDPOINTS = ['api.openai.com', 'api.anthropic.com'];
  ...
  constructor(
    ...
    const securityMode = endpointSecurity || (isBrowser ? 'strict' : 'disabled');
  ...
  private setupGraph() {
    ...
    const builder = new StateGraph(InterviewState)
      ...
      .addConditionalEdges('tools', this.routeFromTools.bind(this), [
        'think',
        'digest_confidentials',
        'digest_concludes',
      ])
      .addConditionalEdges('digest_confidentials', this.routeFromDigest.bind(this), [
        'tools',
        'think',
      ])
  ...
  private async think(state: InterviewStateType) {
    ...
    const priorSystemMessages = (state.messages || []).filter(
  ...
  private llmUpdateTool(state: InterviewStateType) {
    ...
    const wrapperFunc = async (input: any, config: any) => {
      ...
      throw new Error(
        `Wrapper function should not be called. Did Langchain call this automatically?`,
      );
    };
  ...
  private async listen(state: InterviewStateType, config: RunnableConfig) {
    ...
    if (!(msg instanceof AIMessage)) {
      throw new Error(
        `Expected last message to be an AIMessage, got ${msg?.constructor.name}: ${msg}`,
      );
    }
  ...
  private async runTool(
    ...
    const content: string = toolError
   ...
   */
  private makeFieldsData(
    interview: Interview,
    mode: 'normal' | 'conclude' = 'normal',
    counters?: { hint: number; must: number; reject: number },
    fieldNames?: string[],
  ): any[] {
    ...
    for (const fieldName of fieldKeys.reverse()) {
      ...
      const casts = Object.entries(chatfield.casts || {}).map(
        ([name, castData]: [string, any]) => ({
          name,
    ...
    for (const fieldName of Object.keys(interview._chatfield.fields).reverse()) {
      ...
      if (field.specs) {
        ...
        if (field.specs.confidential) {
          specs.push(
            `    - **Confidential**: Do not inquire about this explicitly nor bring it up yourself. Continue your normal behavior. However, if the ${theBob} ever volunteers or implies it, you must record this information.`,
          );
        }
  ...
  async processUpdateTool(interview: Interview, kwargs: Record<string, any>) {
    const definedArgs = Object.keys(kwargs).filter(
      (k) => kwargs[k] !== null && kwargs[k] !== undefined,
    );
  ...
  async go(userInput?: string | null): Promise<string> {
    ...
    if (interrupts.length === 0) {
      throw new Error(
        'ERROR: No interrupts received - this should not happen as the graph should always route to listen',
      );
    }
  ...
  private async digest_confidentials(
  ...
  private buildFieldSchema(fieldName: string, fieldMetadata: any): z.ZodTypeAny {
    ...
    castsDefinitions.value = z
      .string()
      .describe(`The most typical valid representation of a ${this.interview._name} ${fieldName}`);
    ...
    for (const [castName, castInfo] of Object.entries(casts)) {
      ...
      switch (castType) {
        ...
        default:
          throw new Error(
            `Bad type for cast "${castName}" as ${castType}: must be one of int, float, str, bool, list, set, dict, choice`,
          );
      }
    ...
    const fieldSchema = z
      .object({
        value: z
          .string()
          .describe(
            `The most typical valid representation of a ${interview._name} ${fieldName}. Use empty string "" if user wants to skip/leave blank, null if not yet discussed.`,
          ),
        ...castsDefinitions,
      })
  ...
  private mkCastsDefinitions(chatfield: any): Record<string, z.ZodTypeAny> {
    ...
    for (const [castName, castInfo] of Object.entries(casts)) {
      ...
      if (castType === 'choice') {
        ...
        if (!schemaFn) {
          throw new Error(
            `Bad type for cast ${castName}: ${castType}; must be one of ${Object.keys(okPrimitiveTypes).join(', ')}`,
          );
        }
        ...
        if (processed.includes(RESET)) {
          ...
          if (lastPart && lastPart.endsWith(' '.repeat(numTrailing))) {
            parts[parts.length - 1] =
              lastPart.slice(0, -numTrailing) + `${YELLOW_BG}${'·'.repeat(numTrailing)}${RESET}`;
