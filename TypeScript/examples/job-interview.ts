...
function displayResults(interview: any): void {
  ...
  if (interview.experience) {
    console.log('\nExperience Summary:');
    const preview =
      interview.experience.length > 100
  ...
  if (interview.technical_skills) {
    console.log('\nTechnical Skills:');
    const preview =
      interview.technical_skills.length > 100
  ...
  if (interview.preparedness) {
    console.log(
      `  Preparedness: ${interview.preparedness.as_one_assessment || interview.preparedness}`,
    );
  }

  if (interview.cultural_fit) {
    console.log(
      `  Cultural Fit: ${interview.cultural_fit.as_one_assessment || interview.cultural_fit}`,
    );
  }
...
async function main() {
  ...
  try {
    // Run the interview
    const completed = isAuto ? await runAutomated(interview) : await runInteractive(interview);
