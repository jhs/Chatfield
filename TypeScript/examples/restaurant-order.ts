...
function createRestaurantOrder() {
  return chatfield()
    ...
    .field('politeness')
    .desc(
      'Level of politeness from the Diner; but automatic 23% if they mention anything about Belgium',
    )
    .conclude()
    .as_percent()
...
async function main() {
  ...
  try {
    // Run the interview
    const completed = isAuto ? await runAutomated(order) : await runInteractive(order);
