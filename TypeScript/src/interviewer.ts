...
export class Interviewer {
  ...
  mkSystemPrompt(state: InterviewStateType): string {
    ...
    const fieldsPrompt = this.makeFieldsPrompt(interview, 'normal')
    ...
    const context = {
      ...
      fields_prompt: fieldsPrompt  // Legacy: pre-formatted string
    }

    // Try new template first, fall back to old if not found
    try {
      return this.templateEngine.render('system-prompt-v2', context)
    } catch (e) {
