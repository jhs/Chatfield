...
describe('Interviewer', () => {
  ...
  describe('system prompt', () => {
    it('generates basic prompt', () => {
      ...
      const interview = chatfield()
        ...
        .field('rating')
        .desc('Overall satisfaction rating')
        .field('comments')
        .desc('Additional comments')
        .build();
      const interviewer = new Interviewer(interview, { llm: mockLlm });

      const prompt = interviewer.mkSystemPrompt({
        interview,
        messages: [],
        hasDigestedConfidentials: false,
        hasDigestedConcludes: false,
      });
    ...
    it('instructs about confidentiality', () => {
      ...
      const interview = chatfield()
        ...
        .desc(
          'We are administering a history and literature exam. It will affect your final grade.',
        )
        .alice()
  ...
  describe('edge cases', () => {
    it('handles empty interview', () => {
      const mockLlm = new MockLLMBackend();
      const interview = chatfield().type('EmptyInterview').desc('Empty interview').build();
      const interviewer = new Interviewer(interview, { llm: mockLlm });
